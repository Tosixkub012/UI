...
local Fonts = {
    ...
    ["Source Sans Bold"] = {Regular = Enum.Font.SourceSansBold, Medium = Enum.Font.SourceSansBold, Bold = Enum.Font.SourceSansBold},
    ["Source Sans Semibold"] = {Regular = Enum.Font.SourceSansSemibold, Medium = Enum.Font.SourceSansSemibold, Bold = Enum.Font.SourceSansSemibold},
    ["Source Sans"] = {Regular = Enum.Font.SourceSans, Medium = Enum.Font.SourceSans, Bold = Enum.Font.SourceSans},
    ["Arial Bold"] = {Regular = Enum.Font.ArialBold, Medium = Enum.Font.ArialBold, Bold = Enum.Font.ArialBold},
...
function Library:CreateWindow(config)
    ...
    function WindowObject:AddTab(tabConfig)
        ...
        function Elements:Dropdown(dropdownText, options)
            ...
            local hasDesc = descText ~= nil and descText ~= ""
            local DropdownBg, Label = CreateBaseElement(46, hasDesc, descText, dropdownText, isLocked)

            Label.Position = UDim2.new(0, 14, 0.5, hasDesc and -19 or -11)
            Label.TextSize = 13.5
            ...
            DropBtn.Size = UDim2.new(0, 210, 1, 0)
            DropBtn.Position = UDim2.new(1, -224, 0, 0)
            ...
            DropMenu.Size = UDim2.new(0, 210, 0, 0)
            ...
            local function updateMenuPosition()
                ...
                local targetHeight = (matchCount * 30) + 50
                
                local calculatedHeight = math.clamp(targetHeight, 50 + 30, math.min(260, maxHeight))
                
                local margin = 10
                local menuWidth = 210
                local posX = absPos.X + absSize.X - menuWidth
                if posX + menuWidth > currentViewport.X - margin then
                    posX = currentViewport.X - menuWidth - margin
                end
            ...
            function DropdownHandler:Open()
                ...
                local menuTween = TweenService:Create(DropMenu, TweenInfo.new(0.2, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {Size = UDim2.new(0, 210, 0, calculatedHeight)})
                menuTween:Play()
            ...
            function DropdownHandler:Close()
                ...
                local menuTween = TweenService:Create(DropMenu, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0, 210, 0, 0)})
                menuTween:Play()
                menuTween.Completed:Connect(function()
                    DropMenu.Visible = false
                    isAnimating = false
